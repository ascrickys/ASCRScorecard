...
# L. Torgo, Feb 2010
#
# ASC relied on DMwR package
#
# Simple Modifications have been made to the initial SMOTE function
#
# ASC, Rickys Wong, Nov 2020
#
# ---------------------------------------------------

#' @title SMOTE - Imbalanced Data Treatment
#'
#' @description This function handles Imbalanced Data by using SMOTE method.
#'
#' @param form A formula describing the prediction problem
#' @param data A data frame containing the original (imbalanced) data set
#' @param small_prop A new desired proportion for minority class. Default 0.5
#' @param seed A single value, interpreted as an integer. Default 8
#' @param k A number indicating the number of nearest neighbours that are used to generate the new examples of the minority class. Default 5
#'
#' @examples
#' \donttest{
...
#' newData <- ASC_SMOTE(Species ~ ., data, small_prop = 0.5)
#' table(newData$Species)
#' }
#'
#' @export

ASC_SMOTE <- function(form,
                      data,
                      small_prop = 0.5,
                      seed = 8,
                      k = 5, ...) {
  ...
  min_count <- min(table(data[, tgt]))
  max_count <- max(table(data[, tgt]))
  perc.over <- (ceiling(max_count / min_count) - 1) * 100
  if (tgt < ncol(data)) {
    cols <- 1:ncol(data)
  ...
  set.seed(seed = seed)
  newExs <- smote.exs.asc(data[minExs, ], ncol(data), perc.over, k)
  if (tgt < ncol(data)) {
    newExs <- newExs[, cols]
    data <- data[, cols]
  }
  ...
  set.seed(seed = seed)
  newExs = newExs[sample(nrow(newExs), size = floor(small_prop / (1 - small_prop) * max_count - min_count)), ] # select only the number of desired proportion
  newdataset <- rbind(data[selMaj, ], data[minExs, ], newExs)
  return(newdataset)
}


ASC_SMOTE2 <- function(form,
                       data,
                       small_prop = 0.5,
                       seed = 8,
                       k = 5, ...) {
  ...
  newExs = newExs[sample(nrow(newExs), size = floor(small_prop / (1 - small_prop) * max_count - min_count)), ] # select only the number of desired proportion
  newdataset <- rbind(data[selMaj, ], data[minExs, ], newExs)
  return(list(SMOTE_data      = newdataset,
              SMOTE_major     = data[selMaj, ],
              SMOTE_ori_minor = data[minExs, ],
              SMOTE_new_minor = newExs))
}



SMOTE.asc <- function(form,data,
                      perc.over=200,k=5,
                      perc.under=200,
                      learner=NULL,...
)
...
# L. Torgo, Feb 2010
# ---------------------------------------------------
smote.exs.asc <- function(data,tgt,N,k)
  # INPUTS:
